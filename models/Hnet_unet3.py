...
class HNet(nn.Module):
    def __init__(self, in_channels=6, n_classes=3, feature_scale=4, is_deconv=True, is_batchnorm=True, is_ds=True):
        # in_channels 输入通道， n_classes输出通道 is_deconv 是否使用反卷积实现size扩大， is_batchnorm是否加BN层， is_ds是否
    ...
    def forward(self, inputs):
        ...
        # if self.is_ds:
        #     return nn.Sigmoid(final)  # unet++
        # else:
        #     return nn.Sigmoid(final_4)  # 原始的Unet
        if self.is_ds:
            return self.activation(final)  # unet++
        else:
            return self.activation(final_4)  # 原始的Unet
